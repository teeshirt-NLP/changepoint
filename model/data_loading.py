...
class DataLoader:
    ...
    def generate_single_triplet(self):
    ...
    def generate_single_triplet_padded(self, max_nwords=100):
        span = len(self.paragraphs)
        target1, target2 = random.sample(range(span), 2)
    ...
    def generate_batch(self, n, max_nwords=50):
        batch = []
        while len(batch) < n:
            triplet = self.generate_single_triplet_padded(max_nwords)
            if all(len(sublist) == 2 and len(sublist[0]) == max_nwords and len(sublist[1]) == max_nwords for sublist in triplet):
                batch.append(triplet)
        return [batch]


    def generate_single_triplet_raw(self):
        span = len(self.paragraphs)
        target1, target2 = random.sample(range(span), 2)
        ...
        if len(s1) > 3:
            ...
            s1 = s1[s1_index:s1_index + 3]

        if len(s2) > 3:
            s2_index = random.choice(range(len(s2) - 3))
            s2 = s2[s2_index:s2_index + 3]
